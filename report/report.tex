...
The Supervisory and Realtime servers run on both Windows and Linux. The sources of these and the offline template are cross platform. On linux the applications are built from Terminal using \emph{make} command while in the coresponding directory. To recompile on Windows, open the \emph{bh.sln} solution file in Visual Studio. The solution also contains projects \emph{bhLauncer} and \emph{bhSclient}, which use .NET framework and are not cross platform. The binaries will appear in \emph{bin-win} and \emph{bin-linux} folders respectivly. \emph{port2win} includes definitions that are neccessery for compilation of the Linux code on Windows.
