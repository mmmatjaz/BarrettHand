...
\begin{center}
\tt smb://balsa/Interno/Roboti/BarrettHand/BH\_2011 \\ or smb://balsa/Smetje/Matjaz/BHAND
\end{center}
...
The BHserver application allows real time control of the gripper. Once started, it initializes the gripper. The client is connected to the server using UDP protocol, and sends either velocity, position, or desired torque to the server. This enables us to use the graspers internal position control, or design our own controller on client side. The server application applies these values and replies with values read from the gripper (see Figure \ref{sendRec}) All read values are of type double (8 bytes), the PPS data is of type int (4 bytes).
...
\begin{figure}[!h]
    \begin{center}
        \includegraphics[width=0.80\textwidth]{openGL.png}
...
\begin{figure}[!h]
    \begin{center}
        \includegraphics[width=0.3\textwidth]{simmodel2.eps}
		  \includegraphics[width=0.7\textwidth]{simmask.png}
        \caption{Simulink model.} \label{simulink}
    \end{center}
\end{figure}

Figure \ref{simulink} shows the simulink block and its preferences window. In the first tab, control modes and connection parameters are selected. In second tab unused ports can be disabled.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5

%\subsection{Client C++ Wrapper}
%bla
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
...
\section{C++ templates}
On Windows OS one Visual Studio Solution contains all projects. The bhWrapper project contains four .cpp files:
\begin{itemize}
\item
BH.cpp is the main file, program entry is here. It contains the main() function. After it launches the threads, it waits for a keypress to terminate.
\item
bh262.cpp and bh280.cpp contain thread routines for each hand. First, the hand and the software are initialized. The hand now accepts supervisory commands. To control it in real time, the Realtime Mode must be started using RTStart(). Function
RunRealTime() is executed each iteration. It sets velocities or positions and reads the data from the hand sensors.
\item
glutWindow.cpp contains the OpenGL functions to display the BH280 sensor data.
\end{itemize}
On Linux you can use the examples provided by the BarrettHand.
