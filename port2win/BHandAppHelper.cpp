...
int handInitWithMenu(BHand *hand, BHCommunication *communication)
{
	...
	{
		serial = true;
		printf("\t\tThe BarrettHand configuration is set to run a 262 hand\n");
	}
	else if (strcmp(hand->getHardwareDesc()->getModelNumber(), "BH8-280") == 0)
	{
		printf("\t\tThe BarrettHand configuration is set to run a 280 hand\n");
	}
	else
	{
...
/*	printf("\n\n\r\t\tConnect using:");
	...
	{
		printf("\r\t\tConnecting to hand with CAN\n");
