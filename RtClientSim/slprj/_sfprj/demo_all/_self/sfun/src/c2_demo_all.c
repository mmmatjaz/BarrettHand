...
#include "demo_all_sfun.h"
#include "c2_demo_all.h"
#include "mwmathutil.h"
#define CHARTINSTANCE_CHARTNUMBER      (chartInstance.chartNumber)
#define CHARTINSTANCE_INSTANCENUMBER   (chartInstance.instanceNumber)
#include "demo_all_sfun_debug_macros.h"
...
static SFc2_demo_allInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c2_demo_all(void);
static void initialize_params_c2_demo_all(void);
static void enable_c2_demo_all(void);
static void disable_c2_demo_all(void);
static void c2_update_debugger_state_c2_demo_all(void);
static const mxArray *get_sim_state_c2_demo_all(void);
static void set_sim_state_c2_demo_all(const mxArray *c2_st);
static void finalize_c2_demo_all(void);
static void sf_c2_demo_all(void);
...
static void initialize_c2_demo_all(void)
{
  ...
  chartInstance.c2_is_active_c2_demo_all = 0U;
}
...
static const mxArray *get_sim_state_c2_demo_all(void)
{
  ...
  c2_c_u = chartInstance.c2_is_active_c2_demo_all;
  c2_d_y = NULL;
...
static void set_sim_state_c2_demo_all(const mxArray *c2_st)
{
  ...
  real_T c2_d1;
  real_T c2_b_y;
  const mxArray *c2_b_is_active_c2_demo_all;
  uint8_T c2_u0;
  uint8_T c2_c_y;
  real_T *c2_c_Fl;
  real_T *c2_tau;
  ...
  *c2_tau = c2_b_y;
  c2_b_is_active_c2_demo_all = sf_mex_dup(sf_mex_getcell(c2_u, 2));
  sf_mex_import("is_active_c2_demo_all", sf_mex_dup(c2_b_is_active_c2_demo_all),
                &c2_u0, 1, 3, 0U, 0, 0U, 0);
  c2_c_y = c2_u0;
  sf_mex_destroy(&c2_b_is_active_c2_demo_all);
  chartInstance.c2_is_active_c2_demo_all = c2_c_y;
  sf_mex_destroy(&c2_u);
  c2_update_debugger_state_c2_demo_all();
...
static void sf_c2_demo_all(void)
{
  ...
  *c2_b_Fl = c2_Fl;
  ...
  sf_debug_check_for_state_inconsistency(_demo_allMachineNumber_,
...
const mxArray *sf_c2_demo_all_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c2_demo_all_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c2_demo_all_get_autoinheritance_info(void)
{
...
static mxArray *sf_get_sim_state_info_c2_demo_all(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  char *infoEncStr[] = {
    "100 S1x3'type','srcId','name','auxInfo'{{M[1],M[9],T\"Fl\",},{M[1],M[5],T\"tau\",},{M[8],M[0],T\"is_active_c2_demo_all\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 3, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c2_demo_all_get_check_sum(&mxChecksum);
  ...
  if (!sim_mode_is_rtw_gen(S)) {
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        chartAlreadyPresent = sf_debug_initialize_chart(_demo_allMachineNumber_,
          2,
          1,
          1,
          6,
        ...
        if (chartAlreadyPresent==0) {
          ...
          init_script_number_translation(_demo_allMachineNumber_,
            chartInstance.chartNumber);
          sf_debug_set_chart_disable_implicit_casting(_demo_allMachineNumber_,
            chartInstance.chartNumber,1);
          sf_debug_set_chart_event_thresholds(_demo_allMachineNumber_,
            chartInstance.chartNumber,
            0,
            0,
            0);
      ...
      sf_debug_reset_current_state_configuration(_demo_allMachineNumber_,
        chartInstance.chartNumber,chartInstance.instanceNumber);
    }
  }
}

static void sf_opaque_initialize_c2_demo_all(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c2_demo_all();
  initialize_c2_demo_all();
}

static void sf_opaque_enable_c2_demo_all(void *chartInstanceVar)
{
  enable_c2_demo_all();
}

static void sf_opaque_disable_c2_demo_all(void *chartInstanceVar)
{
  disable_c2_demo_all();
}

static void sf_opaque_gateway_c2_demo_all(void *chartInstanceVar)
{
  sf_c2_demo_all();
}

static mxArray* sf_opaque_get_sim_state_c2_demo_all(void *chartInstanceVar)
{
  mxArray *st = (mxArray *) get_sim_state_c2_demo_all();
  return st;
}

static void sf_opaque_set_sim_state_c2_demo_all(void *chartInstanceVar, const
  mxArray *st)
{
  set_sim_state_c2_demo_all(sf_mex_dup(st));
}

static void sf_opaque_terminate_c2_demo_all(void *chartInstanceVar)
{
  ...
  finalize_c2_demo_all();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c2_demo_all(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c2_demo_all(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("demo_all","demo_all",2);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop("demo_all","demo_all",2,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("demo_all","demo_all",2,
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"demo_all","demo_all",2,4);
      sf_mark_chart_reusable_outputs(S,"demo_all","demo_all",2,2);
    }

    sf_set_rtw_dwork_info(S,"demo_all","demo_all",2);
...
static void mdlRTW_c2_demo_all(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "demo_all", "demo_all",2);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c2_demo_all(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway = sf_opaque_gateway_c2_demo_all;
  chartInstance.chartInfo.initializeChart = sf_opaque_initialize_c2_demo_all;
  chartInstance.chartInfo.terminateChart = sf_opaque_terminate_c2_demo_all;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c2_demo_all;
  chartInstance.chartInfo.disableChart = sf_opaque_disable_c2_demo_all;
  chartInstance.chartInfo.getSimState = sf_opaque_get_sim_state_c2_demo_all;
  chartInstance.chartInfo.setSimState = sf_opaque_set_sim_state_c2_demo_all;
  chartInstance.chartInfo.getSimStateInfo = sf_get_sim_state_info_c2_demo_all;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c2_demo_all;
  chartInstance.chartInfo.mdlStart = mdlStart_c2_demo_all;
  chartInstance.chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c2_demo_all;
...
void c2_demo_all_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c2_demo_all(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_demo_all(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_demo_all(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c2_demo_all_method_dispatcher.\n"
