...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_demo_all_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_demo_all_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_demo_all_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_demo_all_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_demo_all_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "demo_all") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_demo_all_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_demo_all_method_dispatcher(SimStruct *S, unsigned int
...
{
  if (!strcmp(machineName,"demo_all")) {
    return(sf_demo_all_method_dispatcher(simstructPtr,chartFileNumber,method,
            data));
  }
...
extern void demo_all_terminator(void);
...
extern void demo_all_initializer(void);
extern void demo_all_register_exported_symbols(SimStruct* S);
extern void demo_all_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  demo_all_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      demo_all_debug_initialize();
    }

    demo_all_initializer();
  }
